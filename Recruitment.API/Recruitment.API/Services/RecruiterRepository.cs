...
namespace Recruitment.API.Services
{
    public class RecruiterRepository : IRecruiterRepository
    {
        ...
        public async Task<ServiceResponse<Interview>> ScheduleInterviewAsync(InterviewScheduleDto scheduleDto)
        {
            ...
            // --- THIS IS THE FIX ---
            // Calculate the round number based on existing interviews in the database
            var existingCount = await _context.Interviews.CountAsync(i => i.application_id == scheduleDto.ApplicationId);
            var nextRound = existingCount + 1;
            // --- END OF FIX ---

            var uniqueMeetingId = Guid.NewGuid().ToString().Substring(0, 8);
            var meetingLink = $"https://meet.jit.si/RoimaInterview-{uniqueMeetingId}";

            var interview = new Interview
            {
                application_id = scheduleDto.ApplicationId,

                // Use the calculated round number, ignore what the frontend sent
            ...
            var interviewersList = new List<User>();
            ...
            foreach (var interviewerId in scheduleDto.InterviewerIds)
            {
                ...
                // Allow continuing even if one ID is invalid, or you could break
                if (!isValidInterviewer)
                {
                    _logger.LogWarning($"Warning: User ID {interviewerId} is not a valid interviewer.");
                    // Depending on requirements, we might want to continue adding valid ones
                    // But for now let's assume we proceed if valid
                }

                // Re-fetch to get object for email
            ...
            try
            {
                string timeString = scheduleDto.ScheduledAt.ToString("f");

                string candidateSubject = $"Interview Scheduled: {application.Job.title}";
                string candidateBody = $@"<h3>Interview Invitation</h3><p>Position: {application.Job.title}</p><p>Round: {nextRound}</p><p>Time: {timeString}</p><p>Link: <a href='{meetingLink}'>{meetingLink}</a></p>";
                await _emailService.SendEmailAsync(application.Candidate.email, candidateSubject, candidateBody);

                foreach (var interviewer in interviewersList)
                {
                    string interviewerSubject = $"New Interview Assigned";
                    string interviewerBody = $@"<h3>New Assignment</h3><p>Candidate: {application.Candidate.first_name}</p><p>Round: {nextRound}</p><p>Time: {timeString}</p><p>Link: <a href='{meetingLink}'>{meetingLink}</a></p>";
            ...
            return new ServiceResponse<Interview> { Data = interview, Message = $"Round {nextRound} interview scheduled successfully." };
        }
        ...
        public async Task<ServiceResponse<List<UserDto>>> GetAvailableHrUsersAsync() { try { var hrRole = await _context.Roles.FirstOrDefaultAsync(r => r.role_name.Equals("HR", StringComparison.OrdinalIgnoreCase)); if (hrRole == null) return new ServiceResponse<List<UserDto>> { Success = false, Message = "HR role definition not found." }; var hrUsers = await _context.Users.Where(u => u.role_id == hrRole.role_id).OrderBy(u => u.first_name).ThenBy(u => u.last_name).Select(u => new UserDto { UserId = u.user_id, Name = $"{u.first_name} {u.last_name}", Email = u.email }).ToListAsync(); return new ServiceResponse<List<UserDto>> { Data = hrUsers }; } catch (Exception ex) { _logger.LogError(ex, "Error fetching HR users."); return new ServiceResponse<List<UserDto>> { Success = false, Message = "Error fetching HR users." }; } }
