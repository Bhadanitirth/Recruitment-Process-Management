    ...
    public class RecruiterRepository : IRecruiterRepository
    {
        ...
        public async Task<ServiceResponse<Candidate>> CreateCandidateAsync(CandidateCreateDto candidateDto, IFormFile cvFile)
        {
            ...
            var candidate = new Candidate
            {
                ...
                // --- MAP COLLEGE NAME ---
                college_name = candidateDto.CollegeName,
                // ------------------------
                created_by_user_id = GetUserId(),
                cv_path = ""
            };

            // Handle CV File saving if provided (logic omitted for brevity, usually handled before saving to DB)
                ...
                using (var workbook = new XLWorkbook(stream))
                {
                    ...
                    foreach (var row in rows)
                    {
                        ...
                        var candidate = new Candidate
                        {
                            ...
                            // --- MAP COLLEGE NAME FROM COLUMN 5 ---
                            college_name = row.Cell(5).Value.ToString().Trim(),
                            // --------------------------------------
                            created_by_user_id = GetUserId()
                        };
            ...
            if (newCandidates.Any())
            {
                using (var transaction = await _context.Database.BeginTransactionAsync())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogError(ex, "Error during bulk candidate creation transaction.");
